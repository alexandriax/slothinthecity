  ...
  useEffect(() => {
    ...
    function frame(timestamp?: number) {
      ...
      if (phaseRef.current === "playing") {
        ...
        const gripping = keys.has("ShiftLeft") || keys.has("ShiftRight"), speed = gripping && energy > 1 ? 6.25 : 3.25; if (wish.lengthSq() > 0) wish.normalize();
        velocity.lerp(wish.multiplyScalar(speed), 1 - Math.exp(-delta * 7)); const before = player.clone(); player.addScaledVector(velocity, delta);
        if (Math.hypot(player.x - 34, player.z + 43) < 23 || Math.abs(player.x) > 112 || Math.abs(player.z) > 112) player.copy(before);
        ...
        energy = Math.max(0, Math.min(100, energy + (gripping && wish.lengthSq() > 0 ? -17 : 5.2) * delta));
        ...
        sloth.animate(gameTime, velocity.length(), gripping); world.animate(gameTime, player, scentRef.current, collected.current);
        ...
        if (gameTime - lastHud > .12) {
          ...
          setHud({ energy, alert, buds: Math.min(collected.current.size, 5), objective: collected.current.size >= 5 ? "Reach the stone sanctuary gate" : "Follow the old bridle trail", prompt, heading: directions[Math.round(head / (Math.PI / 4)) % 8] });
        }
