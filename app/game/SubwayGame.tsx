...
export function SubwayGame({ audio, quality }: SubwayGameProps) {
  ...
  const [transition, setTransition] = useState("Descending beneath Fifth Avenue");
  const [touchCapable, setTouchCapable] = useState(false), toastTimer = useRef<number | null>(null);
  const [hud, setHud] = useState<TransitHud>({ bearing: 0, distance: 20, motion: "IN STATION", objective: "Take a Queens-bound N or R train one stop", objectiveShort: "PLATFORM", prompt: "", promptKey: "", station: "5 AV / 59 ST · 7:12 PM", status: "TRAIN APPROACHING", value: "4S", waypoint: "Queens-bound N / R", wayfinding: true });
  ...
  useEffect(() => {
    ...
    let previousTrainPhase = "AWAY", previousDoorsOpen = false;
    const budget = quality.getRenderBudget(), scene = new THREE.Scene(); scene.background = new THREE.Color("#303936"); scene.fog = new THREE.FogExp2("#303936", .009);
    ...
    const player = stationWorld.spawn.clone(), velocity = new THREE.Vector3(), keys = new Set<string>();
    ...
    function checkpoint(station: SubwayStationId, message: string, waitForNextTrain = false) {
      disposeInterior(); stationWorld ??= createStationWorld(); currentStation = station; stationWorld.setStation(station); player.copy(stationWorld.spawn); velocity.set(0, 0, 0); yaw = 0; pitch = -.04; stationClock = waitForNextTrain ? 18 : 0; boarded = null;
      setTransitStage(station === "FIFTH_AV" ? "FIFTH_AV" : station === "LEXINGTON" ? "LEXINGTON" : "WEST_FARMS"); audio.setScene(station === "WEST_FARMS" ? "west-farms" : "subway-station", { transitionSeconds: 1.15, intensity: .62 }); showTransition(station === "FIFTH_AV" ? "5 Av / 59 St" : station === "LEXINGTON" ? "Lexington Av / 59 St" : "West Farms Sq · E Tremont Av"); showToast(message, 4600);
    }
    function finishRide() {
      if (!boarded) return;
    ...
    function startInterior(option: BoardingOption) {
      ...
      stationWorld.dispose(); stationWorld = null; interiorWorld = new TrainInteriorWorld(scene, textures, journey, hasTouchInput() || quality.getSnapshot().activeLevel === "low" ? "mobile" : "desktop"); player.copy(interiorWorld.spawn); velocity.set(0, 0, 0); yaw = 0; pitch = -.035; keys.clear(); setTransitStage("RIDING"); audio.setScene("moving-train", { transitionSeconds: .7, intensity: .78 }); audio.playTrainChime("doors-closing"); audio.playTrainDoors("close"); showTransition(`${option.route} train · ${option.direction}`); showToast(currentStation === "FIFTH_AV" ? "Stay clear until Lexington Av. Move to the illuminated door when it appears." : "Stay clear at 86 St, 125 St, and E 180 St. Reach the illuminated door at West Farms.", 6200);
    }
    function handleInteriorEvent(event: TrainInteriorEvent | null) {
      ...
      if (event.type === "INTERMEDIATE_STOP") { audio.playTrainChime("arrival"); audio.playTrainDoors("open"); audio.playCrowdBed(.9, 3.2); showToast(`${event.stop} — stand back from the doors while passengers exit`, 3600); }
      else if (event.type === "DESTINATION_READY") { audio.playTrainChime("transfer"); audio.playTrainDoors("open"); showToast(`${event.stop} — move to the illuminated door and exit before it closes`, 4300); }
    ...
    function frame(timestamp?: number) {
      ...
      if (transitStage === "RIDING" && interiorWorld) {
        ...
        if (stationWorld.trainPhase !== previousTrainPhase && stationWorld.trainPhase === "APPROACHING") audio.playTrainArrival(.82);
        if (stationWorld.doorsOpen !== previousDoorsOpen) { audio.playTrainDoors(stationWorld.doorsOpen ? "open" : "close"); if (stationWorld.doorsOpen) audio.playTrainChime("arrival"); else audio.playTrainChime("doors-closing"); }
        ...
        const option = stationWorld.boardingOption(player), target = stationWorld.waypoint, targetX = target.x - player.x, targetZ = target.z - player.z, distance = Math.hypot(targetX, targetZ), ahead = targetX * -Math.sin(yaw) + targetZ * -Math.cos(yaw), side = targetX * Math.cos(yaw) - targetZ * Math.sin(yaw), bearing = THREE.MathUtils.radToDeg(Math.atan2(side, ahead));
        let prompt = "", promptKey = ""; if (option) { prompt = `BOARD ${option.route} · ${option.direction}`; promptKey = "E"; } else if (currentStation === "WEST_FARMS" && distance < 3.1) { prompt = "EXIT TO BRONX ZOO"; promptKey = "E"; }
        if (actionRequested && option) { boarded = option; velocity.set(0, 0, 0); if (option.correct) startInterior(option); else { rideStartedAt = gameTime; rideUntil = gameTime + 5.8; yaw = 0; pitch = -.03; keys.clear(); setTransitStage("RIDING"); audio.setScene("moving-train", { transitionSeconds: .6, intensity: .8 }); showTransition(`${option.route} train · ${option.direction}`); showToast(`Boarded ${option.route} ${option.direction.toLowerCase()} — this is the wrong service`, 3600); } }
        else if (actionRequested && currentStation === "WEST_FARMS" && distance < 3.1) {
          setTransitStage("COMPLETE"); stationWorld.dispose(); stationWorld = null; scene.background = new THREE.Color("#9bb5a0"); scene.fog = new THREE.FogExp2("#b9c7b2", .012); zooWorld = new BronxZooWorld(scene, textures, quality.getSnapshot().profile.foliageDensity); camera.position.copy(zooWorld.cameraPosition); camera.lookAt(zooWorld.cameraTarget); sloth.root.visible = false; audio.setScene("finale", { transitionSeconds: 1.5, intensity: .88 }); audio.playQuestComplete(); showTransition("Bronx Zoo · Asia Gate"); if (document.pointerLockElement) { try { document.exitPointerLock(); } catch {} }
        }
  ...
  return <main className="game-shell subway-shell" data-game-state={stage === "COMPLETE" ? "complete" : "playing"} data-touch-capable={touchCapable ? "true" : "false"} data-motion={hud.motion} data-buds="5" data-level="subway" data-station={stage} data-loaded-world={stage === "RIDING" ? "train-interior" : stage === "COMPLETE" ? "bronx-zoo" : "subway-station"} data-goal-distance={hud.distance.toFixed(1)} data-goal-bearing={hud.bearing.toFixed(1)}>
    ...
    {transition && <div className="world-transition" role="status"><span>Loading world</span><strong>{transition}</strong><i/></div>}
    {stage !== "COMPLETE" && <div className="hud desktop-hud"><section className="mission"><div className="eyebrow">Current objective</div><h2>{hud.objective}</h2><p>{hud.station}</p></section><div className="compass"><div className="eyebrow">Transit · {hud.station}</div><div className="compass-line"><span>N</span><span className="active">{hud.motion}</span><span>R</span></div></div><div className="status"><div className="eyebrow">Service status</div><strong>{hud.status}</strong></div><div className="meters"><div className="motion-state"><span>{hud.motion}</span><small>{stage === "RIDING" ? "Follow the onboard display · clear doors early · reach the illuminated exit at your stop" : "Follow black signs · stairs connect every playable level"}</small></div><div className="meter-row"><span>{stage === "RIDING" ? "Stop" : "Train"}</span><div className="meter-track"><div className="meter-fill" style={{ width: hud.value === "OPEN" ? "100%" : "42%" }}/></div><span>{hud.value}</span></div></div>{hud.prompt && <div className="interaction"><span className="key">{hud.promptKey}</span>{hud.prompt}</div>}<div className="controls-strip"><span>W / A / S / D Walk</span><span>E Interact / Exit</span><span>{stage === "RIDING" ? "Clear doors until your stop" : "Read route + direction"}</span><span>M Sound</span></div></div>}
