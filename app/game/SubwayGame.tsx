...
export function SubwayGame({ audio, quality }: SubwayGameProps) {
  ...
  useEffect(() => {
    ...
    const reflectZooReviewState = (world: BronxZooWorld) => {
      ...
      const objective = released ? "Return to West Farms for the downtown 5 — transfer to N / R later at Lexington Av" : world.objectiveLabel;
      setHud({ bearing: THREE.MathUtils.radToDeg(Math.atan2(dx * Math.cos(yaw) - dz * Math.sin(yaw), dx * -Math.sin(yaw) + dz * -Math.cos(yaw))), distance, motion: released ? "SLOTH RESCUE" : "ZOO EXPLORATION", objective, objectiveShort: quest === "NEED_TICKET" ? "FIND TICKET" : quest === "ENTER_ZOO" ? "ENTER ZOO" : quest === "FIND_SLOTHS" ? "FIND SLOTHS" : "RETURN TRAIN", prompt: hint?.label ?? "", promptKey: hint ? "E" : "", station: released ? "BRONX ZOO · RETURN LEG 1 · WEST FARMS" : "BRONX ZOO · WILDLIFE CONSERVATION CAMPUS", status: quest === "NEED_TICKET" ? "ADMISSION REQUIRED" : quest === "ENTER_ZOO" ? "EXTRA TICKET READY" : quest === "FIND_SLOTHS" ? "EXPLORE THE HABITATS" : "FIRST TRAIN · DOWNTOWN 5", value: `${Math.round(distance)}M`, waypoint: quest === "NEED_TICKET" ? "Visitor with extra ticket" : quest === "ENTER_ZOO" ? "Asia Gate" : quest === "FIND_SLOTHS" ? "Sloth conservation habitat" : "West Farms return entrance", wayfinding: true });
    };
    ...
    function startReturnTransit() {
      ...
      setTransitStage("RETURN_WEST_FARMS"); setReturnLeg("WEST_FARMS"); setZooPhase("RETURN_TRANSIT"); audio.setScene("west-farms", { transitionSeconds: 1.1, intensity: .74 }); audio.playTransitAnnouncement("west_farms_downtown_5_platform", { delaySeconds: .7, dedupeSeconds: 12 }); showTransition("West Farms Sq · Downtown return"); showToast("First take the downtown 5 to Lexington Avenue. Transfer there—not here—to a downtown N or R for Fifth Avenue.", 7200);
      setHud({ bearing: 0, distance: 0, motion: "DESCENDING", objective: "Board the downtown 5 to Lexington Av; the N / R transfer comes next", objectiveShort: "DOWNTOWN 5", prompt: "", promptKey: "", station: "WEST FARMS SQ · RETURN LEG 1 OF 2", status: "DOWNTOWN 5 · THEN N / R", value: `${stationWorld.secondsToTrain}S`, waypoint: "Downtown 5 platform · opposite the arrival side", wayfinding: true });
    }
