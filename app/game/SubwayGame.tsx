...
export function SubwayGame({ audio, quality }: SubwayGameProps) {
  ...
  useEffect(() => {
    ...
    function enterBronxZoo() {
      if (!stationWorld || transitStage === "COMPLETE") return;
    ...
    function frame(timestamp?: number) {
      ...
      if (transitStage === "RIDING" && interiorWorld) {
        ...
        const option = stationWorld.boardingOption(player), boardingHint = stationWorld.boardingHint(player), target = stationWorld.waypoint, targetX = target.x - player.x, targetZ = target.z - player.z, distance = Math.hypot(targetX, targetZ), ahead = targetX * -Math.sin(yaw) + targetZ * -Math.cos(yaw), side = targetX * Math.cos(yaw) - targetZ * Math.sin(yaw), bearing = THREE.MathUtils.radToDeg(Math.atan2(side, ahead));
        let prompt = ""; const promptKey = ""; if (boardingHint) prompt = `WALK THROUGH OPEN ${boardingHint.route} DOORS · ${boardingHint.direction}`; else if (currentStation === "WEST_FARMS" && distance < 4.4) prompt = "WALK UP TO THE BRONX ZOO EXIT";
