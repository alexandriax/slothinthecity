...
export function CharacterShowroom() {
  ...
  return <main className={styles.shell} data-character-debug="true" data-ready-count={readyCount}>
    ...
    <nav className={styles.sceneLinks} aria-label="Direct game debug checkpoints"><span>Review in world</span><Link href="/?debug=zoo">Central Park Zoo</Link><Link href="/?debug=station">Platform</Link><Link href="/?debug=train">N train</Link><Link href="/?debug=transfer">Transfer</Link><Link href="/?debug=bronx">Bronx entry</Link><Link href="/?debug=bronx-sloths">Sloth habitat</Link><Link href="/?debug=return-train-5">Return train</Link><Link href="/?debug=homecoming">Home Grove</Link></nav>
