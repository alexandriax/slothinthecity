...
test("authored vehicle, wildlife, and transit audio is resiliently decoded and event-driven", async () => {
  ...
  for (const path of [
    ...
    ...["fifth_n_platform", "fifth_r_platform", "fifth_n_boarding", "fifth_r_boarding", "lex_arrival_transfer", "lex_5_platform", "lex_5_boarding", "stop_86", "stop_125", "stop_e180", "west_farms_arrival", "west_farms_downtown_5_platform", "west_farms_downtown_5_boarding", "southbound_5_e180", "southbound_5_125", "southbound_5_86", "southbound_5_lexington_transfer", "lex_downtown_n_platform", "lex_downtown_r_platform", "lex_downtown_n_boarding", "lex_downtown_r_boarding", "downtown_nr_fifth_arrival", "stand_clear_doors"].map(name => `/audio/announcements/${name}.mp3`),
  ...
  for (const cue of ["lex_arrival_transfer", "lex_5_platform", "lex_5_boarding", "stop_86", "stop_125", "stop_e180", "west_farms_arrival", "west_farms_downtown_5_platform", "west_farms_downtown_5_boarding", "southbound_5_lexington_transfer", "downtown_nr_fifth_arrival", "stand_clear_doors"]) assert.match(subway, new RegExp(`"${cue}"`));
  assert.match(subway, /`fifth_\$\{route\}_platform`/);
  assert.match(subway, /`fifth_\$\{route\}_boarding`/);
  assert.match(subway, /`lex_downtown_\$\{route\}_platform`/);
  assert.match(subway, /`lex_downtown_\$\{route\}_boarding`/);
